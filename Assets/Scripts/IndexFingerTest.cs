...
public class IndexFingerTest : MonoBehaviour
{
    ...
    void Update ()
    {
        ...
        if (Index1)
        {
            Index1.localRotation = index1Rot * Quaternion.Euler (angle * Vector3.up);
        }
        if (Index2)
        {
            Index2.localRotation = index2Rot * Quaternion.Euler (angle * Vector3.up);
        }
        if (Index3)
        {
            Index3.localRotation = index3Rot * Quaternion.Euler (angle * Vector3.up);
        }
    }
}